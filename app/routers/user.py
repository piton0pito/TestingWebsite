...
from app.models import User, Avatar, CompletedCourses, FavouritesCourse
from app.schemas import UserCreate, UserUpdate, Email, CreateNewPassword, GetUser
...
def get_my_completed_courses(user: User = Depends(verify_access_token), session: Session = Depends(get_session)):
    courses = session.exec(select(CompletedCourses).where(CompletedCourses.user_id == user.id))
    return courses
...
def get_my_favorites_courses(user: User = Depends(verify_access_token), session: Session = Depends(get_session)):
    courses = session.exec(select(FavouritesCourse).where(FavouritesCourse.user_id == user.id))
    return courses
